...
@pytest.mark.integration
def test_aws_earth_search_health() -> None:
    root = Path(__file__).resolve().parent.parent
    data = load_sources_config(root / "config" / "sources.yaml")
    cfg = get_source_by_id(data, "aws-earth-search")
    src = create_source("aws-earth-search", cfg)
    hr = src.health_check(smoke_search=False)
    assert hr.ok, hr.message


@pytest.mark.integration
def test_aws_earth_search_smoke_search() -> None:
    root = Path(__file__).resolve().parent.parent
    data = load_sources_config(root / "config" / "sources.yaml")
    cfg = get_source_by_id(data, "aws-earth-search")
    src = create_source("aws-earth-search", cfg)
...
@pytest.mark.integration
def test_aws_earth_search_search_limited() -> None:
    root = Path(__file__).resolve().parent.parent
    data = load_sources_config(root / "config" / "sources.yaml")
    cfg = get_source_by_id(data, "aws-earth-search")
    src = create_source("aws-earth-search", cfg)
